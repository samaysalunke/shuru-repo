...
def main():
    ...
    # ==========================================
    with col_chat:
        # Display all messages (no container wrapper)
        ...
        # Chat input renders AFTER all messages (at bottom)
        ...
        if os.path.exists('knowledge_base.json'):
            try:
                ...
                for cs in case_studies_list[:15]:  # Limit to 15 items
                    ...
                    display_case_study_item(client, industry, "💼")
                
                if not case_studies_list:
                    st.info("No case studies found")
            except Exception as e:
                st.error("Error loading case studies")
        else:
